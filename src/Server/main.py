...
from src.Server.Components.GridGeneration import generate_grid, visualize_grid, visualize_clearance_grid, visualize_grid_with_path, \
    remove_x_from_grid, find_obstacle_coords, create_obstacle_grid, create_clearance_grid, analyze_clearance_grid
from src.Server.Pathfinding.Pathfinding import *
from src.Server.Utils.px_conversion import realCoordinates
from src.Server.Utils.path_conversion import *
...
def calculate_homography_matrix(source_points=None):
    ...
    destination_points = np.array([[0, 0], [170, 0], [170, 125], [0, 125]],
                                  dtype=np.float32)  # THESE ARE WRITTEN IN x,y FORMAT
...
def detect_robot_and_balls(frame):
    ...
    balls_result = DetectAllBalls(frame)
...
def send_distance_to_robot(communicator: RobotCommunicator, remaining_distance, current_heading, current_position):
    communicator.update_distance(distance=remaining_distance, current_heading=current_heading,
...
def main():
    ...
    while True:
        ...
        print(
            f"Robot position (center point): {robot_base_position}    Robot heading point: {robot_tip_position}    Ball positions: {ball_positions}")

        if robot_base_position is None:
            print("No robot detected keep looking")
            continue

        robot_base_position = realCoordinates(robot_height_cm, camera_height_cm,
                                              robot_base_position)  # this returns in format (x, y)
        robot_base_position = (
        int(robot_base_position[0]), int(robot_base_position[1]))  # this returns in format (x, y)
        ...
        flattened_ball_positions = [(int(ball[0]), int(ball[1])) for ball_array in ball_positions for ball in
                                    ball_array]
        ...
        if orange_ball_index is not None:
            ...
            closest_ball_position = min(flattened_ball_positions,
                                        key=lambda x: np.linalg.norm(np.array(x) - np.array(robot_base_position)))
            print(f"Closest ball is the ball at position: {closest_ball_position}")

        closest_ball_position = realCoordinates(4, camera_height_cm,
                                                closest_ball_position)  # this returns in format (x, y)

        closest_ball_updated_position = (
        int(closest_ball_position[1]), int(closest_ball_position[0]))  # this is in format (y, x)
        ...
        path = a_star_fms_search(standard_grid, clearance_grid, robot_base_updated_position,
                                 closest_ball_updated_position, min_clearance)
        ...
        if simplified_path:
            print(
                f"Sending data to robot - Current position: {robot_base_position}, Current heading: {robot_heading}, Target{simplified_path[0]}, Number of waypoints: {len(simplified_path)}")
            ...
            while True:
                ...
                if confirmation == "update_heading":
                    ...
                    remaining_distance = getDistance(simplified_path[1],
                                                     robot_base_position)  # This is in px. This is the current distance between the robot and the next waypoint
                    while remaining_distance > 3:
                        initial_position = (
                        int(robot_base_position[0]), int(robot_base_position[1]))  # Initial robot position
                        updated_position = update_robot_position()  # Update robot position from sensors
                        current_position = (
                        int(updated_position[0]), int(updated_position[1]))  # Convert to integer coordinates

                        traveled_distance = getDistance(initial_position,
                                                        updated_position)  # Distance traveled in the last loop
                        remaining_distance = getDistance(simplified_path[1],
                                                         current_position)  # Updated remaining distance

                        current_heading = update_robot_heading()  # Updated robot heading
                        print(
                            f"Traveled distance: {traveled_distance}, Remaining distance: {remaining_distance}, Current heading: {current_heading}")
                    ...
                    if np.linalg.norm(np.array(robot_base_position) - np.array(closest_ball_position)) < 10:
                        ...
                        communicator.send_data(robot_base_position, robot_heading, normalized_travel_path[0][1],
                                               normalized_travel_path[0][0], len(normalized_travel_path))
